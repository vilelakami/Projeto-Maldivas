...
void init_dialogue_manager(DialogueManager* manager) {
    init_dialogues(manager->dialogues);
    manager->dialogue_index = 0;
    manager->dialogue_finished = false;
}
...
void movimento_eng(NPC_ENGENHEIRO* npc_engenheiro, NPC_DIRETOR* npc_chefe) {
    static bool movendo = true;

    if (movendo) {
        npc_engenheiro->direcao = DIRECAO_CIMA;
        npc_engenheiro->y -= npc_engenheiro->velocidade * (1.0 / FPS);
